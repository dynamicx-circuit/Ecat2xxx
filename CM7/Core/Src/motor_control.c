...
//    float rotor_speed = speed * (float) motor[id - 1].motor_decelration;
    float rotor_speed = speed ;

    //pid计算并发送数据
    pid_calc(pid_struct, (float) motor[id - 1].motor_measure.speed_rpm, 15.0f);
    set_motor_current((int16_t) pid_struct->pos_out, id);

    //将数据打印到vofa
    printf_fdata2vofa(motor[id - 1].motor_measure.speed_rpm, speed, pid_struct->pos_out, rotor_speed);
